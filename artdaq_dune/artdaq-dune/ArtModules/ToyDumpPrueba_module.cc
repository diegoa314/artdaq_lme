 ...
 */
class prueba::ToyDump : public art::EDAnalyzer
{
public:
	/**
...
prueba::ToyDump::ToyDump(fhicl::ParameterSet const& pset)
	: EDAnalyzer(pset)
...
prueba::ToyDump::~ToyDump() {}

void prueba::ToyDump::analyze(art::Event const& evt)
{
	...
	TLOG(TLVL_INFO) << "Run " << evt.run() << ", subrun " << evt.subRun()
		...
		<< " fragment(s) of type PRUEBA";

	for (const auto& frag : fragments)
	{
		PruebaFragmento bb(frag);

		TLOG(TLVL_INFO) << demo::fragmentTypeToString(static_cast<demo::detail::FragmentType>(frag.type())) << " fragment " << frag.fragmentID()
			<< " w/ seqID " << frag.sequenceID() << " and timestamp " << frag.timestamp();

		//if (frag.hasMetadata())
		//{
			PruebaFragmento::Metadato const* md =
				frag.metadata<PruebaFragmento::Metadato>();
			TLOG(TLVL_INFO) << "Fragment metadata: " << std::showbase << "Board serial number = "
				<< md->serial << ", sample bits = "
				<< md->n_adc_bits;
	...
	//	}
		...
		if (num_adcs_to_write_ >= 0)
		{
			...
			if (binary_mode_) {
				...
				{
					output.write((char*)(bb.dataBeginADCs() + i_adc), sizeof(PruebaFragmento::adc_t));
				}
				output.close();
			}
			else {
				std::ofstream output(output_file_name_, std::ios::out | std::ios::app);
				output << demo::fragmentTypeToString(static_cast<demo::detail::FragmentType>(frag.type())) << "\t" << frag.fragmentID();
		...
		if (num_adcs_to_print_ >= 0)
		{
			...
			for (int idx = 0; idx < rows; ++idx)
			{
				...
				for (uint32_t jdx = 0; jdx < columns_to_display_on_screen_; ++jdx)
				{
					...
					o << (int)bb.adc_value(adc_counter);
					++adc_counter;
					TLOG(TLVL_INFO) << bb.adc_value(adc_counter);
...
DEFINE_ART_MODULE(prueba::ToyDump)
